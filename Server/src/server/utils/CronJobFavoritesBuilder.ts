...
import FavoriteStore, {
  FavoriteStoreDocument,
} from "@schema/favoriteStore.schema";
    ...
    try {
      ...
      if (userInfo) {
        const items = await MainInstance.GetFavoritesInfos(
          schedule[userInfo.subscription]
        );

        var FavoriteStoreIds: mongoose.Types.ObjectId[] = [];
        if (userInfo.favoriteStores.length !== items.length) {
          for (let x = 0; x < items.length; x++) {
            let favoriteStore = await FavoriteStore.findOne({
              item_id: items[x].item_id,
            });
            if (favoriteStore) FavoriteStoreIds.push(favoriteStore._id);
            else {
        ...
        for (const item of items) {
          ...
          if (originalQuantity && originalQuantity.quantity !== item.quantity) {
            ...
            await FavoriteStore.findByIdAndUpdate(originalQuantity._id, {
              $set: { oldQuantity: originalQuantity.quantity },
            });
            await FavoriteStore.findByIdAndUpdate(originalQuantity._id, item, {
              new: true,
            });
